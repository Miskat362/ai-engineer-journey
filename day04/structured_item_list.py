...
response = client.models.generate_content(
    model="gemini-3.6-flash",
    contents="""Analyze these two reviews:
    1. "Inception - mind-bending masterpiece, 9/10"
    2. "The Room - so bad it's hilarious, 2/10 but I enjoyed the chaos"
    """,
    config=types.GenerateContentConfig(
        response_mime_type="application/json",
...
for r in response.parsed.reviews:
    print(f"{r.title}: {r.sentiment}, {r.rating_out_of_10}/10")